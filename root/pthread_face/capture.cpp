/* Authors : Aaresh Bachana , Abhilash Manjunath, Harshil Sheth, Shrivathsa Murthy 
   ...
   Referrences: http://docs.opencv.org/2.4/modules/contrib/doc/facerec/tutorial/facerec_video_recognition.html
		https://github.com/jetsonhacks/JHHC-SR04/tree/master/src
...
struct timespec ts_main,ts_motor, ts_recog, ts_finder, ts_gps, ts_us;

static const char GPS_LOG[] = "gps_log";
static const char NAV_LOG[] = "nav_log";

static volatile int ctrlC = 1;

void interruptHandler(int dummyVar) {
    ctrlC = 0;
}
using namespace cv;
using namespace std;
...
double deadline_ms_recognition = 9000;   // deadline for all tasks. 
double deadline_ms_finder = 1200;
double deadline_ms_sensor = 450;
double deadline_ms_gps   = 6300;
...
void createTrackbars()
{
	...
	sprintf( TrackbarName, "G_MIN %d", G_MIN);
	sprintf( TrackbarName, "G_MAX %d", G_MAX);
...
void* path_finder(void*)
{
	...
	capture.open(0);
	double rt;
	 clock_gettime(CLOCK_REALTIME, &ts_finder);
	pthread_mutex_unlock(&frame_show_lock);
	rt = (((double)ts_finder.tv_sec * 1000.0) + ((double)((double)ts_finder.tv_nsec /1000000.0)))-
                ((((double)ts_main.tv_sec * 1000.0) + ((double)((double)ts_main.tv_nsec /1000000.0))));
	while(1)
	{
		...
		clock_gettime(CLOCK_REALTIME, &frame_time_finder);
			...
			printf("End-#2---Frame jitter Path Finder = %f\n", ave_framedt_jitter);
	 		printf("Request time for Path Finder = %fms\n",rt);
			capture.release();
			return NULL;
}
...
void * face_recognition(void *)
{
	...
	string box_test;
	double rt;
	clock_gettime(CLOCK_REALTIME, &ts_gps);
	 clock_gettime(CLOCK_REALTIME, &ts_recog);
        rt = (((double)ts_recog.tv_sec * 1000.0) + ((double)((double)ts_recog.tv_nsec /1000000.0)))-
                ((((double)ts_main.tv_sec * 1000.0) + ((double)((double)ts_main.tv_nsec /1000000.0))));
    for(;;) 
	{
		pthread_mutex_lock(&frame_cap_lock);
    ...
    printf("Request time for Face Recognition  = %fms\n",rt);
    cap.release();
    return NULL;
}
...
gpioExport(hcsr04Trigger1);
	...
	clock_gettime(CLOCK_REALTIME, &ts_us);
	rt = (((double)ts_us.tv_sec * 1000.0) + ((double)((double)ts_us.tv_nsec /1000000.0)))-
                ((((double)ts_main.tv_sec * 1000.0) + ((double)((double)ts_main.tv_nsec /1000000.0))));
	while(1)
	{
		...
		*/
		if (duration<10 )
		{
			sem_post(&semaphore_motor_sensor);
	...
	printf("Request time for ultrasonic sensor  = %fms\n",rt);
	return NULL;
}

bool checkTaskStatus(const char *fileName)
{
	FILE *fp;
        long fileSize;
        char *buffer;
	...
	fseek(fp, 0, SEEK_END);
        fileSize = ftell(fp);   // calculate file size
...
void* gps(void*){
	...
	*/
	...
	double frameTime1, prevFrameTime1;
        printf("Entering gps thread \n");	
	double rt;
	//double gpsJitter, gpsAvg;
	//printf("./gmaps.py starting.. ret val = %d\n",system("./gmaps.py"));
	//sleep(5);
	//printf("gmaps.py running..\n");
	clock_gettime(CLOCK_REALTIME, &ts_gps);
	rt = (((double)ts_gps.tv_sec * 1000.0) + ((double)((double)ts_gps.tv_nsec /1000000.0)))-
		((((double)ts_main.tv_sec * 1000.0) + ((double)((double)ts_main.tv_nsec /1000000.0))));
//	printf("Request time for GPS = %fms\n",rt);
	for (;;){
		...
		diff1 = frameTime1 - prevFrameTime1;
        	...
        	//if(  ( frameCount1 > 2 ) &&  (diff1 > deadline )){
            	...
            	//	}
		if ( ctrlC == 0){
			printf("Break out of GPS\n");
			break;
			}
		}
		//printf("End-#5---Average execution time  =  %fms\n",gpsAvg);
		//printf("End-#5---Gps jitter = %f\n",gpsJitter);
		printf("Request time for GPS = %fms\n",rt);
    		//printf(" fps - %f\n", avgFps1); 
}
...
{
	double rt;
	clock_gettime(CLOCK_REALTIME, &ts_motor);
	rt = (((double)ts_motor.tv_sec * 1000.0) + ((double)((double)ts_motor.tv_nsec /1000000.0)))-
                ((((double)ts_main.tv_sec * 1000.0) + ((double)((double)ts_main.tv_nsec /1000000.0))));
	while(1)
	{
		...
		if( ctrlC == 0)
		{
			printf("Breaking out of motor_control \n");
			break; 
		}
    	}		
	printf("Request time for motor control = %fms\n",rt);
}
...
int main( int argc, char** argv )
{
    ...
    printf("./gmaps.py starting.. \nReturn status  = %s\n",(system("./gmaps.py")== 0) ? "SUCCESS\nRunning script..": "FAILURE");
    sleep(5);
    printf("gmaps.py running..\n");
	...
	pthread_create(&threads[4], &rt_sched_attr, gps, NULL);
